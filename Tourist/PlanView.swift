...
struct ItineraryView: View {
    ...
    @State private var showHotelSheet = false
    ...
    var body: some View {
        NavigationStack {
                        ...
                        VStack(spacing: 16) {
                            ForEach(Array(tripPlanner.itineraryDays.enumerated()), id: \.element.id) { index, day in
                                VStack(spacing: 8) {
                                    ...
                                    // Hotels button - completely separate from navigation link
                                    Button {
                                        selectedLocation = day.mainLocation
                                        showHotelSheet = true
                                    } label: {
            ...
            .navigationBarTitleDisplayMode(.inline)
            .sheet(isPresented: $showHotelSheet) {
                NavigationStack {
                    HotelListView(location: selectedLocation)
                }
            }
            .onAppear {
